...
describe('StartSyncOrchestration', () => {
	...
	it('should run fetch before migrate then init when config does not exist', async () => {
		...
		expect(mockMigrateExistingAgentsToBridge.execute).toHaveBeenCalledWith({
			workspaceRoot: '/mock/root',
			selectedAgentId: 'cursor',
		});
		expect(mockInitializeProject.execute).toHaveBeenCalledWith({
			workspaceRoot: '/mock/root',
			force: false,
		});
	...
	it('should NOT run migration or init when config does not exist and user cancels selector', async () => {
		...
		expect(mockStatusBar.update).toHaveBeenCalledWith(
			SyncStatus.ERROR,
