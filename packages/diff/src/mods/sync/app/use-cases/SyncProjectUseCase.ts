...
export class SyncProjectUseCase {
     ...
     */
    async execute({ rules, sourcePath, targetPath }: SyncProjectRequestDTO): Promise<SyncResultDTO> {
        ...
        try {
            for (const ruleDto of rules) {
                ...
                const actions = await this.interpreter.interpret(rule, { sourceRoot: sourcePath, targetRoot: targetPath });
