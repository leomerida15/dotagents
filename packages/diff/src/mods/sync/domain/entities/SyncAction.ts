    ...
    public async execute(fileSystem: any): Promise<void> {
        ...
        switch (this.type) {
            case ActionType.COPY:
                if (!this.sourcePath) throw new Error(`Source path missing for COPY to ${this.targetPath}`);
                await fileSystem.copy(this.sourcePath, this.targetPath);
                break;
            case ActionType.WRITE:
                const content = typeof this.contentValue === 'string'
                    ? this.contentValue
            ...
            case ActionType.MOVE:
                if (!this.sourcePath) throw new Error(`Source path missing for MOVE to ${this.targetPath}`);
                await fileSystem.copy(this.sourcePath, this.targetPath);
