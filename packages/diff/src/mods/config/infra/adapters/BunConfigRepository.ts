...
export class BunConfigRepository implements IConfigRepository {
    private readonly DOT_AI_FOLDER: string;
    ...
    constructor({
        ...
        this.DOT_AI_FOLDER = dotAiFolder;
        this.SYNC_FILE = syncFile;
    }

    public async save(config: Configuration): Promise<void> {
        const aiPath = join(config.workspaceRoot, this.DOT_AI_FOLDER);
        ...
        const data: PersistedConfigData = {
            ...
            agents: config.agents.map(agent => ({
                ...
                inbound: agent.inboundRules.map(r => ({ from: r.from, to: r.to, format: r.format })),
                outbound: agent.outboundRules.map(r => ({ from: r.from, to: r.to, format: r.format }))
            }))
        };
    ...
    public async load(workspaceRoot: string): Promise<Configuration> {
        const syncPath = join(workspaceRoot, this.DOT_AI_FOLDER, this.SYNC_FILE);
