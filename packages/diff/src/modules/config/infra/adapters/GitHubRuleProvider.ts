...
export class GitHubRuleProvider implements IRuleProvider {
	...
	constructor({ baseUrl }: GitHubRuleProviderProps = {}) {
		this.GITHUB_RAW_BASE_URL =
			baseUrl ?? 'https://raw.githubusercontent.com/snor/dotagnets-rules/main';
	}
	...
	public async fetchAgentDefinitions(): Promise<AgentProps[]> {
		try {
			...
			console.warn(
				'Could not fetch rules from GitHub, using local fallback if available.',
				error,
			);
			return [];
		}
