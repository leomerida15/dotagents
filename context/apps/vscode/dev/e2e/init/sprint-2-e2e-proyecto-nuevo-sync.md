...
- **Next:** Sprint 5 (Regresión rutas) reutiliza este flujo para afirmar YAML del motor en `.agents/.ai/rules/`.
...
- Verificar en disco: existe `.agents/.ai/rules/{agentId}.yaml`, existe `.agents/.ai/state.json` (o equivalente), estructura `.agents` esperada.
...
- [x] Verificación de `.agents/.ai/rules/{agentId}.yaml` en el test.
- [x] Verificación de state/manifest y estructura `.agents`.

### Evidencia (verificación local)
- Fecha: 2026-03-17
- Comando: `cd apps/vscode && DOTAGENTS_E2E_AGENT=opencode DOTAGENTS_E2E_SUITE=full DOTAGENTS_E2E_FIXTURE=newProjectWithOpencode bun run test:e2e`
