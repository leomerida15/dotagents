...
- Los YAML del motor (instalados/descargados) viven en **`.agents/.ai/rules/{agentId}.yaml`**.
- **`.agents/rules/`** es el puente para contenido sincronizado (p. ej. `.md`), no para copias del YAML de definición del agente.
- Los E2E comprueban que no exista `{agentId}.yaml` duplicado bajo `.agents/rules/`.
