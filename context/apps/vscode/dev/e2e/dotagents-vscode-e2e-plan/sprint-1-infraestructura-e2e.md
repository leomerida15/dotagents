...
## Context
Para analizar el comportamiento de la extensión en otra ventana del IDE en modo debug hace falta una batería de tests E2E. El primer paso es disponer de la infraestructura: `@vscode/test-e2e`, un script que lance el Extension Development Host, y una carpeta de tests con al menos un test que abra un workspace y verifique que la extensión está activa.
...
## Pasos a ejecutar
- Añadir dependencia `@vscode/test-e2e` en `apps/vscode/package.json`.
- Crear configuración de launch para E2E (o reutilizar/adaptar la de “Extension” para que el test runner use el mismo host).
- Crear carpeta `apps/vscode/e2e/` con `runTest.ts` / entrypoint que use `runTests` de `@vscode/test-e2e`. Todos los tests E2E se guardan en `apps/vscode/e2e/`.
...
- Añadir script `test:e2e` en `package.json` que ejecute la batería E2E.
